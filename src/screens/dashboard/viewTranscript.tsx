...
const ViewTranscript: FC<Props> = ({navigation}) => {
  ...
  const attemptedCreditRow = [{id: '1', code: 'Attempted Credit:', title: '4'}];

  const creditEarnRow = [{id: '1', code: 'Credit Earn:', title: '4'}];

  const gpaRow = [{id: '1', code: 'GPA/4.0:', title: '3'}];

  const TableHeader = () => (
    <View
      style={[styles.tableBlock, {borderBottomWidth: 1, borderColor: '#ccc'}]}>
      <View style={{flex: 4, backgroundColor: '#2372E9', padding: 20}}>
        <Text style={{fontWeight: '400', fontSize: 20, color: 'white'}}>
          Final Examination Results for the 2022/2023 and Semester 1 for
          Registered Courses
        </Text>
      </View>
    </View>
    ...
    <View
      ...
      <View style={styles.column}>
        {row.map((item, itemIndex) => (
          <Text key={itemIndex} style={[styles.label, styles.marginBottom]}>
            {item.code}
          </Text>
        ))}
      </View>
      ...
      <View style={styles.column}>
        {row.map((item, itemIndex) => (
          <Text key={itemIndex} style={[styles.value, styles.marginBottom]}>
            {item.title}
          </Text>
        ))}
      </View>
    ...
    <ScrollView style={{padding: 20}}>
      ...
      <TouchableOpacity style={styles.downloadButton} onPress={() => {}}>
        <Text style={styles.downloadButtonText}>
          Download
          <Text> </Text>
          <Icons size={15} icon={IconType.FILE} color="#fff" />
        </Text>
      </TouchableOpacity>
    </ScrollView>
  );
